...
export class ApprovalfeedetailComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  public approvalfeedetailData: any = [];
  ...
  //#region  form submit
  onFormSubmit(approvalfeedetailForm: NgForm, isValid: boolean) {
    ...
    this.approvalfeedetailData = this.JSToNumberFloats(approvalfeedetailForm);

    const usersJson: any[] = Array.of(this.approvalfeedetailData);
    if (this.params != null) {
      // call web service
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              ...
              $('#applicationDetail').click();
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/inquiry/subapplicationmainlist/applicationmaindetail/' + this.param + '/administrationdetail/' + this.param + '/approvalfeedetail/', this.param, parse.id], { skipLocationChange: true });
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
