...
export class ApprovaldeviationdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  // variable
  public approvaldeviationdetailData: any = [];
  ...
  //#region  form submit
  onFormSubmit(approvaldeviationdetailForm: NgForm, isValid: boolean) {
    ...
    this.approvaldeviationdetailData = approvaldeviationdetailForm;
    const usersJson: any[] = Array.of(this.approvaldeviationdetailData);
    if (this.params != null) {
      // call web service
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              ...
              $('#applicationDetail').click();
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/application/banberjalanapplicationmain/applicationmaindetail/' + this.param + '/' + this.pageType + '/approvaldetail/' + this.param + '/' + this.model.level_status + '/' + 'banberjalan' + '/deviationdetail/', this.param, parse.id, 'banberjalan'], { skipLocationChange: true });
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
