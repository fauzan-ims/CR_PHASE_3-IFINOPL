...
export class ApprovalexpenselistComponent extends BaseComponent implements OnInit {
  // get param from url
              ...
              res => {
                ...
                if (parse.result === 1) {
                  if (this.checkedList.length == J + 1) {
                    ...
                    $('#datatableApprovalexpenselist').DataTable().ajax.reload();
                  }
                } else {
                  this.showSpinner = false;
  ...
  btnSaveList() {
    ...
    const getAmount = $('[name="p_expense_amount"]')
      .map(function () { return $(this).val(); }).get();
