...
export class ApprovalfinancialrecapitulationdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  public approvalfinancialrecapitulationdetailData: any = [];
  public listapprovalfinancialrecapitulationdetail: any = [];
  ...
  private APIControllerApplicationApprovalfinancialrecapitulationDetail: String = 'ApplicationFinancialrecapitulationDetail';
  ...
  private APIRouteForLookupForApprovalfinancialrecapitulationDetail: String = 'GetRowsLookupForFinancialrecapitulationDetail';
  ...
  loadData() {
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIControllerApplicationApprovalfinancialrecapitulationDetail, this.APIRouteForGetRows).subscribe(resp => {
          const parse = JSON.parse(resp)
          this.listapprovalfinancialrecapitulationdetail = parse.data;
          if (parse.data != null) {
            this.listapprovalfinancialrecapitulationdetail.numberIndex = dtParameters.start;
          }
  ...
  //#region  form submit
  onFormSubmit(approvalfinancialrecapitulationdetailForm: NgForm, isValid: boolean) {
    ...
    this.approvalfinancialrecapitulationdetailData = approvalfinancialrecapitulationdetailForm;
    const usersJson: any[] = Array.of(this.approvalfinancialrecapitulationdetailData);
    if (this.params != null) {
      // call web service
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              $('#applicationDetail').click();
              $('#applicationApprovalfinancialrecapitulation').click();
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/inquiry/subapplicationmainlist/applicationmaindetail/' + this.param + '/surveydetail/' + this.param + '/approvalfinancialrecapitulationdetail/', this.param, parse.code], { skipLocationChange: true });
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  btnBack() {
    ...
    $('#datatableApplicationApprovalfinancialrecapitulation').DataTable().ajax.reload();
  }
  ...
  btnLookupSysGeneralSubcode() {
    ...
    $('#datatablelookupSysGeneralSubcode').DataTable({
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIControllerSysGeneralSubcode, this.APIRouteForLookupForApprovalfinancialrecapitulationDetail).subscribe(resp => {
          const parse = JSON.parse(resp);
  ...
  btnSelectAllLookup() {
    ...
    for (let J = 0; J < this.checkedLookup.length; J++) {
      ...
      this.dalservice.Insert(this.dataTamp, this.APIControllerApplicationApprovalfinancialrecapitulationDetail, this.APIRouteForInsert)
        .subscribe(
          res => {
            const parse = JSON.parse(res);
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              $('#datatableApplicationApprovalfinancialrecapitulationDetail').DataTable().ajax.reload();
  ...
  btnDeleteAll() {
    ...
    swal({
      ...
      if (result.value) {
        ...
        this.dalservice.Delete(this.dataTamp, this.APIControllerApplicationApprovalfinancialrecapitulationDetail, this.APIRouteForDelete)
          .subscribe(
            res => {
  ...
  btnSaveList() {
    this.showSpinner = true;
    this.listapprovalfinancialrecapitulationdetail = [];
    ...
    while (i < getID.length) {
      ...
      while (i < getFromAmount.length) {
        ...
        while (i < getToAmount.length) {
          ...
          this.listapprovalfinancialrecapitulationdetail.push(
    ...
    //#region web service
    // tslint:disable-next-line:max-line-length
    this.dalservice.Update(this.listapprovalfinancialrecapitulationdetail, this.APIControllerApplicationApprovalfinancialrecapitulationDetail, this.APIRouteForUpdate)
      .subscribe(
        res => {
          const parse = JSON.parse(res);
          if (parse.result === 1) {
            this.dalservice.ExecSp(this.dataTamp, this.APIControllerApplicationApprovalfinancialrecapitulationDetail, this.APIRouteForExecSp)
              .subscribe(
                ress => {
                  ...
                  if (parses.result === 1) {
                    this.callGetrow();
                    this.showSpinner = false;
                    $('#btnSaveApplicationApprovalfinancialrecapitulation').click();
                    $('#applicationDetail').click();
                    this.showNotification('bottom', 'right', 'success');
                    $('#datatableApplicationApprovalfinancialrecapitulationDetail').DataTable().ajax.reload();
                  } else {
