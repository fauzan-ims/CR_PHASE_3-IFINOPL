...
export class KaroseriaccessorieslistComponent extends BaseComponent implements OnInit {
    ...
    for (let i = 0; i < this.lookupservice.length; i++) {
      if (this.lookupservice[i].selectedLookup) {
        this.checkedLookup.push({
          ...
          'is_subject_to_purchase': this.lookupservice[i].is_subject_to_purchase,
        });
      }
    }
    ...
    (function loopAddBudgetMaintenanceGroup() {
      if (J < th.checkedLookup.length) {
        th.dataTamp = [{
          ...
          'p_is_subject_to_purchase': th.checkedLookup[J].is_subject_to_purchase,
        }];
