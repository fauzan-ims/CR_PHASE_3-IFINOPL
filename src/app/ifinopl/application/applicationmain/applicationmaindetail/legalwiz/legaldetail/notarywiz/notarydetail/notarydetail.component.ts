        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              ...
              $('#applicationDetail').click();
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/application/subapplicationmainlist/applicationmaindetail/' + this.param + '/legaldetail/' + this.param + '/' + this.branch + '/notarydetail/', this.param, parse.id, this.branch], { skipLocationChange: true });
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
