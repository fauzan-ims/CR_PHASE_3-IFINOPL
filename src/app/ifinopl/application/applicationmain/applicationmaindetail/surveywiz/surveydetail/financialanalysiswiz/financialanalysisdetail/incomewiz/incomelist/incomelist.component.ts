...
export class IncomelistComponent extends BaseComponent implements OnInit {
  // get param from url
              ...
              res => {
                ...
                if (parse.result === 1) {
                  if (this.checkedList.length == J + 1) {
                    ...
                    $('#datatableIncomeList').DataTable().ajax.reload();
                  }
                } else {
                  this.showSpinner = false;
  ...
  btnSaveList() {
    ...
    const getAmount = $('[name="p_income_amounts"]')
      .map(function () { return $(this).val(); }).get();
