...
@Component({
  selector: 'app-invoicehold',
  templateUrl: './invoiceholdlist.component.html'
})
export class InvoiceHoldlistComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  public listinvoicehold: any = [];
  public tampOPLStatus: String;
  ...
  private APIRouteForGetRows: String = 'GetRowsForInvoiceHold';

  private RoleAccessCode = 'R00024690000001A';
  ...
  ngOnInit() {
    ...
    this.tampOPLStatus = 'ALL';
    this.loadData();
  }

  //#region Status
  OPLStatus(event) {
    this.tampOPLStatus = event.target.value;
    $('#datatableinvoicehold').DataTable().ajax.reload();
  }
  //#endregion ttatus
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        dtParameters.paramTamp.push({
          ...
          'p_opl_status': this.tampOPLStatus
        });

        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          const parse = JSON.parse(resp);
          this.listinvoicehold = parse.data;
          if (parse.data != null) {
            this.showSpinner = false;
            this.listinvoicehold.numberIndex = dtParameters.start;
