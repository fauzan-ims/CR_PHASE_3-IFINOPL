...
export class ReferencedetailComponent extends BaseComponent implements OnInit {
  ...
  public referencedetailData: any = [];
  ...
  ngOnInit() {
    ...
    if (this.paramss != null) {
      ...
      this.model.relation_type = 'REFFERENCE';
      this.model.reference_type_code = 'REFFERENCE';
  ...
  //#region form submit
  onFormSubmit(referencedetailForm: NgForm, isValid: boolean) {
    ...
    this.referencedetailData = this.JSToNumberFloats(referencedetailForm);
    const usersJson: any[] = Array.of(this.referencedetailData);
    if (this.paramss != null) {
      // call web service
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              $('#clientDetail').click();
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/client/subclientpersonallist/clientpersonaldetail/' + this.param + '/' + this.params + '/' + this.type + '/' + this.from + '/' + this.page + '/referencedetail', this.param, this.params, this.type, this.from, this.page, parse.id], { skipLocationChange: true });
            } else {
              this.swalPopUpMsg(parse.data);
            }
          },
