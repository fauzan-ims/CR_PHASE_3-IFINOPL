...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './sp1list.component.html'
})

export class Sp1listComponent extends BaseComponent implements OnInit {
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          ...
          // end checkall
          this.listsp1 = parse.data;
          if (parse.data != null) {
            this.listsp1.numberIndex = dtParameters.start;
          }
  ...
  btnProcess() {
    this.isBreak = false;
    this.checkedList = [];
    for (let i = 0; i < this.listsp1.length; i++) {
      if (this.listsp1[i].selected) {
        this.checkedList.push(this.listsp1[i].code);
      }
    }
              ...
              res => {
                ...
                if (parse.result === 1) {
                  if (J + 1 === this.checkedList.length) {
                    ...
                    $('#datatableSp1List').DataTable().ajax.reload();
                  }
                } else {
                  this.isBreak = true;
                  this.showSpinner = false;
  ...
  selectAllTable() {
    for (let i = 0; i < this.listsp1.length; i++) {
      this.listsp1[i].selected = this.selectedAll;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAll = this.listsp1.every(function (item: any) {
