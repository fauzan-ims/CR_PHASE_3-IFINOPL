...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './sp2list.component.html'
})

export class Sp2listComponent extends BaseComponent implements OnInit {
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          ...
          this.listsp2 = parse.data;
          if (parse.data != null) {
            this.listsp2.numberIndex = dtParameters.start;
          }
  ...
  btnProcess() {
    this.isBreak = false;
    this.checkedList = [];
    for (let i = 0; i < this.listsp2.length; i++) {
      if (this.listsp2[i].selected) {
        this.checkedList.push(this.listsp2[i].code);
      }
    }
              ...
              res => {
                ...
                if (parse.result === 1) {
                  if (J + 1 === this.checkedList.length) {
                    ...
                    $('#datatableSp2List').DataTable().ajax.reload();
                  }
                } else {
                  this.isBreak = true;
                  this.showSpinner = false;
  ...
  selectAllTable() {
    for (let i = 0; i < this.listsp2.length; i++) {
      this.listsp2[i].selected = this.selectedAll;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAll = this.listsp2.every(function (item: any) {
