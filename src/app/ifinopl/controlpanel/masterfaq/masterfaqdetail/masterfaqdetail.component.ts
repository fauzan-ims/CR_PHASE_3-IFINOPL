...
export class MasterfaqdetailComponent extends BaseComponent implements OnInit {
  ...
  private RoleAccessCode = 'R00022630000001A'; // role access 
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/controlpanel/subfaqlist/faqdetail', parse.id]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/controlpanel/subfaqlist']);
