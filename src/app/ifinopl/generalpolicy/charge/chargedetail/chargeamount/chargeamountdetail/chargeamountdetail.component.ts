...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './chargeamountdetail.component.html'
})

export class ChargeamountdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  public chargeamountdetailData: any = [];
  ...
  private APIController: String = 'MasterChargesAmount';
  ...
  //#region  form submit
  onFormSubmit(chargeamountdetailForm: NgForm, isValid: boolean) {
    ...
    this.chargeamountdetailData = chargeamountdetailForm;
    if (this.chargeamountdetailData.p_is_fn_override == null) {
      this.chargeamountdetailData.p_is_fn_override = false;
      this.chargeamountdetailData.p_fn_override_name = '';
    }
    this.chargeamountdetailData = this.JSToNumberFloats(chargeamountdetailForm);
    const usersJson: any[] = Array.of(this.chargeamountdetailData);
    if (this.params != null) {
      // call web service
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/generalpolicy/subchargelist/chargeamountdetail', this.param, parse.code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/generalpolicy/subchargelist/chargedetail', this.param]);
