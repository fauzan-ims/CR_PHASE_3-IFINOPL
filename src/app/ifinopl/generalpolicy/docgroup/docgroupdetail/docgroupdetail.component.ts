...
@Component({
  selector: 'app-docgroupdetail',
  templateUrl: './docgroupdetail.component.html' // GetRowsForLookupForDocgroup
})
export class DocgroupdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  param = this.getRouteparam.snapshot.paramMap.get('id');

  // variable
  public docgroupData: any = [];
  public listdocgroup: any = [];
  ...
  private APIController: String = 'MasterDocumentGroup';
  private APIControllerDetail: String = 'MasterDocumentGroupDetail';
  ...
  private APIRouteForLookupGeneral: String = 'GetRowsForLookupForDocgroup';
  private APIRouteForGetRows: String = 'GETROWS';
  private APIRouteForLookup: String = 'GetRowsForLookup';
  private APIControllerDoc: String = 'SysGeneralDocument'; // multiple
  private APIRouteForLookupDoc: String = 'GetRowsForLookupForMultipleDocGroup';
  private APIRouteForGetRow: String = 'GETROW';
  private APIRouteForUpdate: String = 'UPDATE';
  private APIRouteForInsert: String = 'INSERT';
  private APIRouteForDelete: String = 'DELETE';

  private RoleAccessCode = 'R00016830000000A'; // role access 
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIControllerDetail, this.APIRouteForGetRows).subscribe(resp => {
          ...
          for (let i = 0; i < parse.data.length; i++) {
            ...
            this.listdocgroup = parse.data;
          }

          this.listdocgroup = parse.data;
          if (parse.data != null) {
            this.listdocgroup.numberIndex = dtParameters.start;
          }
  ...
  btnSelectAllLookup() {
    ...
    for (let J = 0; J < this.checkedLookup.length; J++) {
      ...
      this.dataTampPush.push({
        'p_document_group_code': this.param,
        'p_general_doc_code': this.checkedLookup[J],
        'p_is_required': '1'
      });
  ...
  //#region  form submit
  onFormSubmit(docgroupForm: NgForm, isValid: boolean) {
    ...
    this.docgroupData = docgroupForm;
    if (this.docgroupData.p_is_active == null) {
      this.docgroupData.p_is_active = false;
    }
    const usersJson: any[] = Array.of(this.docgroupData);
    if (this.param != null) {
      // call web service
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/generalpolicy/subdocgrouplist/docgroupdetail', parse.code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/generalpolicy/subdocgrouplist']);
  ...
  btnLookupGeneral() {
    ...
    $('#datatableLookupGeneral').DataTable({
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIControllerGeneral, this.APIRouteForLookupGeneral).subscribe(resp => {
          const parse = JSON.parse(resp);
  ...
  btnSelectRowGeneral(code: String, description: String) {
    ...
    this.model.document_group_type_name = description;
  ...
  btnDeleteAll() {
    this.checkedList = [];
    for (let i = 0; i < this.listdocgroup.length; i++) {
      if (this.listdocgroup[i].selected) {
        this.checkedList.push(this.listdocgroup[i].id);
      }
    }
  ...
  selectAllTable() {
    for (let i = 0; i < this.listdocgroup.length; i++) {
      this.listdocgroup[i].selected = this.selectedAllTable;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAllTable = this.listdocgroup.every(function (item: any) {
