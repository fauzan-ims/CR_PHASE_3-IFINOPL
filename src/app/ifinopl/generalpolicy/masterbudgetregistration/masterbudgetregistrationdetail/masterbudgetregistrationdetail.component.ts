...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './masterbudgetregistrationdetail.component.html'
})

export class MasterbudgetregistrationdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  param = this.getRouteparam.snapshot.paramMap.get('id');

  // variable
  public NumberOnlyPattern = this._numberonlyformat;
  public budgetRegistrationData: any = [];
  ...
  onFormSubmit(budgetreplacementdetaildetailForm: NgForm, isValid: boolean) {
    ...
    this.budgetRegistrationData = budgetreplacementdetaildetailForm;

    if (this.budgetRegistrationData.p_is_active == null) {
      this.budgetRegistrationData.p_is_active = false;
    }

    const usersJson: any[] = Array.of(this.JSToNumberFloats(this.budgetRegistrationData));
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/generalpolicy/subbudgetregistrationlist/budgetregistrationdetail', parse.code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  //#region button add
  btnAdd() {
    this.route.navigate(['/generalpolicy/subbudgetregistrationlist/budgetregistrationdetaildetail', this.param]);
  }
  //#endregion button add

  //#region button edit
  btnEdit(codeEdit: string) {
    this.route.navigate(['/generalpolicy/subbudgetregistrationlist/budgetregistrationdetaildetail', this.param, codeEdit]);
  }
  //#endregion button edit

  //#region button back
  btnBack() {
    this.route.navigate(['/generalpolicy/subbudgetregistrationlist']);
    $('#datatablebudget').DataTable().ajax.reload();
  }
