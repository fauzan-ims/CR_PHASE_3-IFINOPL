...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './masterbudgetreplacementdetail.component.html'
})

export class MasterbudgetreplacementdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  param = this.getRouteparam.snapshot.paramMap.get('id');

  // variable
  public NumberOnlyPattern = this._numberonlyformat;
  public budgetReplacementData: any = [];
  ...
  private RoleAccessCode = 'R00023990000001A'; // role access 
  ...
  onFormSubmit(budgetreplacementdetaildetailForm: NgForm, isValid: boolean) {
    ...
    this.budgetReplacementData = budgetreplacementdetaildetailForm;

    if (this.budgetReplacementData.p_is_active == null) {
      this.budgetReplacementData.p_is_active = false;
    }

    const usersJson: any[] = Array.of(this.JSToNumberFloats(this.budgetReplacementData));
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/generalpolicy/subbudgetreplacementlist/budgetreplacementdetail', parse.code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  //#region button add
  btnAdd() {
    this.route.navigate(['/generalpolicy/subbudgetreplacementlist/budgetreplacementdetaildetail', this.param]);
  }
  //#endregion button add

  //#region button edit
  btnEdit(codeEdit: string) {
    this.route.navigate(['/generalpolicy/subbudgetreplacementlist/budgetreplacementdetaildetail', this.param, codeEdit]);
  }
  //#endregion button edit

  //#region button back
  btnBack() {
    this.route.navigate(['/generalpolicy/subbudgetreplacementlist']);
