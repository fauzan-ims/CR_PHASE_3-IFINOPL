...
export class TypedetailComponent extends BaseComponent implements OnInit {
  ...
  private APIController: String = 'MasterHeType';
  ...
  private RoleAccessCode = 'R00020110000000A'; // role access 
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/he/typedetail', this.typeData.p_code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data)
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/he/subtypelist']);
    $('#datatableHeTypeList').DataTable().ajax.reload();
  }
