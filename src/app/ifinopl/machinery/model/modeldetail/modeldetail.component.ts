...
export class ModeldetailComponent extends BaseComponent implements OnInit {
  ...
  private APIController: String = 'MasterMachineryModel';
  ...
  private RoleAccessCode = 'R00020170000000A'; // role access 
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/machinery/submodellist/modeldetail', this.modelData.p_code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/machinery/submodellist']);
    $('#datatableMachineryModelList').DataTable().ajax.reload();
  }
