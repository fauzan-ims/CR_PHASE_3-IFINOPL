...
@Component({
  selector: 'app-reportdailyormonthlytransaction',
  templateUrl: './reportdailyormonthlytransaction.component.html'
})
export class ReportdailyormonthlytransactionComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  private APIControllerReportSys: String = 'SysReport';
  private APIRouteForDownload: String = 'getReport';
  private APIRouteForValidasi: String = 'ExecSpValidasiTanggal';
  ...
  onFormSubmit(rptForm: NgForm, isValid: boolean, print_option: any) {
    ...
    this.dataTampValidate = [{
      'p_date': this.reportData.p_date,
      'p_month': this.reportData.p_month,
    ...
    this.dalservice.ExecSp(this.dataTampValidate, this.APIControllerReportSys, this.APIRouteForValidasi)
      .subscribe(
        res => {
          const parse = JSON.parse(res);
          if (parse.result === 1) {
