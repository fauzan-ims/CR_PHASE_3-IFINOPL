...
@Component({
    selector: 'app-reportoutstandinginvoice',
    templateUrl: './reportoutstandinginvoice.component.html'
})
export class ReportoutstandinginvoiceComponent extends BaseComponent implements OnInit {
    // get param from url
    ...
    private APIRouteForLookupCustomer: String = 'GetRowsForLookupCustomer';
    ...
    btnSelectRowSysBranch(code: String, description: String) {
        ...
        this.model.customer_code = undefined;
        this.model.customer_name = undefined;
    ...
    //#region SysBranch Lookup
    btnLookupCustomer() {
        $('#datatableLookupCustomer').DataTable().clear().destroy();
        $('#datatableLookupCustomer').DataTable({
            'pagingType': 'first_last_numbers',
            'pageLength': 5,
            'processing': true,
            'serverSide': true,
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                // end param tambahan untuk getrows dynamic
                this.dalservice.Getrows(dtParameters, this.APIControllerAgreementMain, this.APIRouteForLookupCustomer).subscribe(resp => {
                    const parse = JSON.parse(resp);
                    this.lookupcustomer = parse.data;
                    if (parse.data != null) {
                        this.lookupcustomer.numberIndex = dtParameters.start;
                    }
                    callback({
                        draw: parse.draw,
                        recordsTotal: parse.recordsTotal,
    ...
    btnSelectRowCustomer(code: String, name: String) {
        this.model.customer_code = code;
        this.model.customer_name = name;
        $('#lookupModalCustomer').modal('hide');
    }
