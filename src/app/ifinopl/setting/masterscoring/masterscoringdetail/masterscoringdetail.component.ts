...
@Component({
  selector: 'app-root',
  templateUrl: './masterscoringdetail.component.html'
})
export class MasterscoringdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  param = this.getRouteparam.snapshot.paramMap.get('id');
  params = this.getRouteparam.snapshot.paramMap.get('id2');

  // variable
  public masterscoringData: any = [];
  ...
  private APIController: String = 'MasterScoring';
  private APIControllerMasterScoringItem: String = 'MasterScoringItem';
  private APIControllerDimension: String = 'MasterScoringDimension';
  ...
  private APIRouteForSync: String = 'ExecSpForSyncLos';

  private RoleAccessCode = 'R00016890000000A'; // role access 
  ...
  //#region form submit
  onFormSubmit(masterscoringForm: NgForm, isValid: boolean) {
    ...
    this.masterscoringData = masterscoringForm;
    if (this.masterscoringData.p_is_active == null) {
      this.masterscoringData.p_is_active = false;
    }
    const usersJson: any[] = Array.of(this.masterscoringData);
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/setting/submasterscoringlist/masterscoringdetail', parse.code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data)
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/setting/submasterscoringlist']);
  ...
  btnSync(code: string) {
    ...
    this.dataTampPush = [{
      'p_scoring_code': code,
      'p_scoring_type': this.model.scoring_reff_type,
      //'p_reff_survey_category_code': reff_survey_category_code,
      'action': 'getResponse'
    }];
    // param tambahan untuk button Sync dynamic
    ...
    swal({
      ...
      if (result.value) {
        // call web service
        this.dalservice.ExecSpScr(this.dataTampPush, this.APIControllerMasterScoringItem, this.APIRouteForSync)
          .subscribe(
            ress => {
                ...
                if (this.dataTamp.length > 0) {
                  ...
                  for (let JJ = 0; JJ < this.dataTamp.length; JJ++) {
                    this.listdimension.push({
                      'p_scoring_code': this.param,
                      'p_reff_item_code': this.dataTamp[JJ].code,
                      'p_reff_item_name': this.dataTamp[JJ].description
                    });
                  }
                    ...
                    .subscribe(
                      ressss => {
                        ...
                        if (parsesss.result === 1) {
                          this.showSpinner = false;
                          this.showNotification('bottom', 'right', 'success');
                          $('#datatableMasterScoringDetail').DataTable().ajax.reload();
                        } else {
                          this.showSpinner = false;
  ...
  btnSelectRowGeneral(code: String, name: String) {
    this.model.scoring_reff_type = code;
    this.model.scoring_reff_name = name;
  ...
  btnSelectRowDimension(description: String) {
    ...
    $('#datatableLookupSelectDimension').modal('hide');
  }
