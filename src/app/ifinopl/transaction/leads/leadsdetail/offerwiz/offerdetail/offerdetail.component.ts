...
export class OfferdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  private APIController: String = 'ProspectOffer';
      ...
      .subscribe(
        res => {
          ...
          if (parse.data.length !== 0) {
            ...
            if (parsedata.is_using_simulation == '1') {
              parsedata.is_using_simulation = true;
            }
            else {
              parsedata.is_using_simulation = false;
            }
          }

          if (parsedata.object_type == null || parsedata.object_type === '') {
            this.isType = false;
          }
          else {
            this.isType = true;
  ...
  //#region form submit
  onFormSubmit(offerForm: NgForm, isValid: boolean) {
    ...
    this.offerData = this.JSToNumberFloats(offerForm);

    if (this.offerData.p_first_date == '' || this.offerData.p_first_date == null) {
      this.offerData.p_first_date = undefined;
    }

    if (this.offerData.p_last_date == '' || this.offerData.p_last_date == null) {
      this.offerData.p_last_date = undefined;
    }

    if (this.offerData.p_followup_next_action_date == '' || this.offerData.p_followup_next_action_date == null) {
      this.offerData.p_followup_next_action_date = undefined;
    }

    if (this.offerData.p_admin_amount === '' || this.offerData.p_admin_amount == null) {
      this.offerData.p_admin_amount = 0;
    }

    if (this.offerData.p_provision_amount === '' || this.offerData.p_provision_amount == null) {
      this.offerData.p_provision_amount = 0;
    }

    if (this.offerData.p_insurance_amount === '' || this.offerData.p_insurance_amount == null) {
      this.offerData.p_insurance_amount = 0;
    }

    if (this.offerData.p_other_fee_amount === '' || this.offerData.p_other_fee_amount == null) {
      this.offerData.p_other_fee_amount = 0;
    }

    if (this.offerData.p_eff_rate === '' || this.offerData.p_eff_rate == null) {
      this.offerData.p_eff_rate = 0;
    }

    if (this.offerData.p_flat_rate === '' || this.offerData.p_flat_rate == null) {
      this.offerData.p_flat_rate = 0;
    }

    const usersJson: any[] = Array.of(this.offerData);
    // call web service
  ...
  //#region object type lookup
  btnLookupObjectType() {
    $('#datatableLookupObjectType').DataTable().clear().destroy();
    $('#datatableLookupObjectType').DataTable({
      'pagingType': 'first_last_numbers',
      'pageLength': 5,
      'processing': true,
      'serverSide': true,
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIControllerGeneralSubcode, this.APIRouteForLookup).subscribe(resp => {
          const parse = JSON.parse(resp);
          this.lookupobjecttype = parse.data;

          if (parse.data != null) {
            this.lookupobjecttype.numberIndex = dtParameters.start;
          }
  ...
  btnSelectRowObjectType(code: String, name: String) {
    this.model.object_type = code;
    this.model.object_type_name = name;
    $('#lookupModalObjectType').modal('hide');
  }
  //#endregion object type lookup
