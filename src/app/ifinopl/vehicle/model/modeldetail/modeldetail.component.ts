...
export class ModeldetailComponent extends BaseComponent implements OnInit {
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/vehicle/submodellist/modeldetail', this.modelData.p_code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/vehicle/submodellist']);
    $('#datatableVehicleModelList').DataTable().ajax.reload();
  }
