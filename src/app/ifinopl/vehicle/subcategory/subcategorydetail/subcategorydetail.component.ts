...
export class SubcategorydetailComponent extends BaseComponent implements OnInit {
  ...
  private APIController: String = 'MasterVehicleSubcategory';
  ...
  private RoleAccessCode = 'R00019990000000A'; // role access 
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/vehicle/subsubcategorylist/subcategorydetail', this.subcategoryData.p_code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/vehicle/subsubcategorylist']);
    $('#datatableVehicleSubCategoryList').DataTable().ajax.reload();
  }
