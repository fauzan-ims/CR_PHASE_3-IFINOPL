...
export class TypedetailComponent extends BaseComponent implements OnInit {
  ...
  private APIController: String = 'MasterVehicleType';
  ...
  private APIRouteLookupUnit: String = 'GetRowsForLookup';

  private RoleAccessCode = 'R00020030000000A'; // role access 
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/vehicle/subtypelist/typedetail', this.typeData.p_code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data);
            }
  ...
  //#region button back
  btnBack() {
    this.route.navigate(['/vehicle/subtypelist']);
    $('#datatableVehicleTypeList').DataTable().ajax.reload();
  }
