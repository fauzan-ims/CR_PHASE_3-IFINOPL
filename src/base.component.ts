...
export abstract class BaseComponent extends BaseService {
    ...
    protected iframePublishPath = this.AllModUrl('publishPathIFINOPL');
    //#endregion
        ...
        service.ExecSpSys(dataRoleTamp, ApiController, APIRouteForGetRole)
            .subscribe(
                res => {
                    ...
                    const domElements = document.querySelectorAll('[data-role-textbox]');
                    for (let j = 0; j < domElements.length; j++) {
                        // tslint:disable-next-line:no-shadowed-variable
                        const elementAttr = domElements[j].getAttribute('data-role-textbox');
                        if (rolecodeHeader.indexOf(elementAttr) === -1) {
                            $('[data-role-textbox = "' + elementAttr + '"]').attr('disabled', true)
                        }
                    }
                    // end hide element
                },
    ...
    protected _emailformat = "[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$";
    // tslint:disable-next-line:member-ordering
    ...
    protected _textonlyformat = '^[a-zA-Z ]*$';
    protected _alfaNumericformat = '^[ a-zA-Z0-9.,-]*$';
    ...
    protected _confdialog = 'You won\'t be able to revert this!';
            ...
            $('[amount]').on('paste keydown keyup blur focus', function (event: any) {
                ...
                if (event.type === 'keydown') {
                    ...
                    if (!((event.keyCode >= 48 && event.keyCode <= 57) || (event.keyCode >= 96 && event.keyCode <= 105)
                        ...
                        || event.keyCode == 8 || event.keyCode == 9 || event.keyCode == 46 || event.keyCode == 110 || event.keyCode == 190
    ...
    //#region RoundingFunction
    RoundingFunction(roundType: any, roundAmount: any, amount: any) {
        let amountAfterRounding = 0;
        if (roundType === 'DOWN') {
            amountAfterRounding = Math.floor(amount / roundAmount) * roundAmount;
        } else if (roundType === 'UP') {
            amountAfterRounding = Math.ceil(amount / roundAmount) * roundAmount;
        } else {
            amountAfterRounding = Math.round(amount / roundAmount) * roundAmount;
        }
