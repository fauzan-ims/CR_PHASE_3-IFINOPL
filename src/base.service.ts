...
export abstract class BaseService {
  ...
  //#region encrypt APIKey
  protected encryptAPIKey(data: any) {
    try {
      let _key = CryptoJS.enc.Utf8.parse("A2IVRJSTDHTP1CJV");
      let _iv = CryptoJS.enc.Utf8.parse("V56E7QYOGREMAIII");
      let hash = Md5.hashStr("/api/"+data.toLowerCase()+this.uid);
      return this.encryptLogic(this.encryptLogic(CryptoJS.AES.encrypt(hash + "&cdct" + formatDate(new Date(), 'yyyy-MM-dd HH:mm:ss', 'en_US'), _key, {
        keySize: 128,
        iv: _iv,
